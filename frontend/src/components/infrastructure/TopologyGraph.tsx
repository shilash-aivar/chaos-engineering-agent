...
type TopologyNode = { id: string; label: string; x: number; y: number; tier: string }
...
export function TopologyGraph({ nodes, edges, blastPath = [], title = 'Blast radius' }: TopologyGraphProps) {
  ...
  return (
    <Card>
      <CardHeader className="pb-2">
        ...
        <p className="text-xs text-muted-foreground">Dependency graph from live infrastructure snapshot</p>
      </CardHeader>
      <CardContent>
            ...
            >
              <div
                className={`rounded-md border px-2 py-1 text-center text-[10px] font-medium shadow-sm ${
                  isBlast(node.id)
                    ? 'border-red-team bg-red-team/25 text-red-team ring-2 ring-red-team/30'
                    : tierColors[node.tier] ?? tierColors.standard
                }`}
              >
                {node.label}
              </div>
        ...
        {blastPath.length > 0 && (
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            <Badge variant="outline">{blastPath.slice(0, 4).join(' → ')}</Badge>
          </div>
        )}
      </CardContent>
    </Card>
